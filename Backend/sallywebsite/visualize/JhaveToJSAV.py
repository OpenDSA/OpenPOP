...
# 3- run the java command given by prof. Tom : "java -cp jhavepop-support.jar:jdom.jar:jaxen.jar:. exe.memorymanager.memorymanager foobar X foo.xml"
#os.system() 
...
############################################## Get the code sent from the front end to  the back end ###################################################################
# the student code should come from the server
studentCode= """ Remove all A's from the list // gridsize 1 6 java 0.3
...
foo.write(studentCode)
foo.write("</value_entered></textarea></input_panel>")
foo.close()
####################### Run the Java command to produce the snap shots file ##########################################################################################

status = os.system("java -cp jhavepop-support.jar:jdom.jar:jaxen.jar:. exe.memorymanager.memorymanager foobar X foo.xml")
print status
#commands.getstatusoutput("java -cp jhavepop-support.jar:jdom.jar:jaxen.jar:. exe.memorymanager.memorymanager foobar X foo.xml ")


################################################## Parsing the .sho (snap shots) file and writting down the tokens file #################################################

#tree = xml.parse('C:\\Sally\\Research\\JhavePOP\\jhavepop-standalone\\foobar.sho') # parse an XML file by name
